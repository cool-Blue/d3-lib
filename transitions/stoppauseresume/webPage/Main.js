...
function createSVG( id ) {
  var update = d3.select( "#" + id).selectAll("svg").data([1]);
  update.enter().append("svg")
      .attr("class", "widget");
    return update;
}
...
function WidgetChart( ) {
  this.init = function( id ) {
     ...
     WidgetChart.prototype.init.call( this, id, [] );

    var me = this;

    this.plot = plot.bind(this, me.coordSys)
...
WidgetChart.prototype = new function(){
  var x = 0, w = 80, h = 128;
  this.init = function( id ) {
    this.id = id;
    this.svg = createSVG(this.id);
    this.coordSys = createCoordSys(this.id, x, 0, w, h, 15, 4);
    x += w;
    createObject(this.coordSys);
    return this;
  }
